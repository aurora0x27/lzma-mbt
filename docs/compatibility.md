...
| XZ ARM64 BCJ filter | yes | yes | yes | decode+encode |
...
- Extra root-package `filters` (Delta / BCJ) are applied around the codec and are validated before encode/decode. XZ container-native filters are handled separately as Block Header filters before LZMA2: Delta (ID `0x03`), x86 BCJ (ID `0x04`), and ARM BCJ (ID `0x07`), and ARM64 BCJ (ID `0x0A`) each form a two-filter chain `Delta -> LZMA2` / `x86 -> LZMA2` / `ARM -> LZMA2` / `ARM64 -> LZMA2`. The root-package extra BCJ and the container x86 BCJ use the full liblzma `simple/x86.c` transform (MSByte / `prev_mask`), verified byte-for-byte against `lzma_bcj_x86_encode/decode` vectors; the container ARM BCJ mirrors `simple/arm.c` (24-bit offset of `0xEB`-prefixed words) and the container ARM64 BCJ mirrors `simple/arm64.c` (BL 26-bit immediate and in-range ADRP page immediates), both verified byte-for-byte against `lzma_bcj_arm64_*` vectors and `xz --arm` / `xz --arm64` files. Other container filter IDs (PowerPC `0x05`, IA-64 `0x06`, ARM-Thumb `0x08`, SPARC `0x09`) are still `UnsupportedFeature`.
