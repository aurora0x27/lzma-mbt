...
- Extra root-package `filters` (Delta / BCJ) are applied around the codec and are validated before encode/decode. XZ container-native filters are handled separately as Block Header filters before LZMA2: Delta (ID `0x03`) and x86 BCJ (ID `0x04`) each form a two-filter chain `Delta -> LZMA2` / `x86 -> LZMA2`. Both the root-package extra BCJ and the container x86 BCJ use the full liblzma `simple/x86.c` transform (MSByte / `prev_mask`), verified byte-for-byte against `lzma_bcj_x86_encode/decode` vectors. Other container filter IDs are still `UnsupportedFeature`.
