...
Single-stream and concatenated `.xz` files with one or more Blocks per Stream, LZMA2 filter, optional Delta filter before LZMA2, and Check None / CRC32 / CRC64 / SHA-256. Other container filters are out of scope (raise `UnsupportedFeature`).
...
Delta filter ID is `0x03` with a 1-byte distance property encoded as `distance - 1`. LZMA2 filter ID is `0x21` with a 1-byte dictionary property. The supported chain is either `LZMA2` or `Delta -> LZMA2`.
...
This encoder writes exactly one Block with compressed-size and uncompressed-size present (`Block Flags` bits 6 and 7), either one LZMA2 filter or Delta followed by LZMA2, then Index and Footer.
...
- Chains longer than `Delta -> LZMA2` → `UnsupportedFeature`
- Delta as the final filter or malformed Delta/LZMA2 properties → `Data`
...
Empty payload (0 Blocks, empty Index); single Block; 2+ Blocks; compressed size VLI of one byte; check field length 0/4/8/32; dictionary property for 4 KiB vs preset-6 8 MiB; Delta distances 1/4/256; truncation in a later Block's Header / Data / Check; two and three concatenated Streams; Stream Padding of 4 and 8 null bytes between Streams.
