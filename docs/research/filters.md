# Filters (Delta, BCJ x86 / ARM / ARM64)

## Scope

Standalone Delta plus the full x86 and ARM BCJ transforms (`simple/x86.c`, `simple/arm.c`), and XZ Block-Header chains `Delta -> LZMA2`, `x86 -> LZMA2`, `ARM -> LZMA2`, and `ARM64 -> LZMA2`. Root-package extra filters wrap LZMA/LZMA2/XZ payloads via `EncodeOptions.filters` / `DecodeOptions.filters`. Other ISA BCJ filters (ARM-Thumb/PowerPC/IA64/SPARC) remain unsupported; unknown XZ filter IDs decode as `UnsupportedFeature`.
...
Filter IDs (for `.xz` headers, not this API enum): Delta `0x03`, x86 `0x04`, ARM `0x07`, ARM64 `0x0A`, LZMA2 `0x21`.

## Reference implementation

`src/liblzma/simple/x86.c`, `simple/arm.c`, `simple/arm64.c`, and `simple_coder.c` (xz 5.8), `src/liblzma/delta/delta_*.c`. Byte-level x86 and ARM64 vectors are generated with the system `lzma_bcj_x86_encode/decode` and `lzma_bcj_arm64_encode/decode` (xz 5.8.3) and embedded in the tests; ARM is verified against an `xz --arm` reference file plus self round-trips because liblzma exposes no standalone `lzma_bcj_arm_encode/decode`.
...
XZ container-native pre-filters are separate from root-package extra filters: `xz` decodes a Block chain `Delta -> LZMA2` / `x86 -> LZMA2` / `ARM -> LZMA2` / `ARM64 -> LZMA2` by LZMA2-decoding the filtered bytes, then applying the pre-filter's inverse (Delta-decode, or x86/ARM/ARM64 decode) before Block Check verification. `xz.encode_xz(..., delta_distance=n)` / `(..., bcj_x86=true)` / `(..., bcj_arm=true)` / `(..., bcj_arm64=true)` apply the chosen pre-filter before LZMA2 and write the two filter records (`0x03` then `0x21`, `0x04` then `0x21`, `0x07` then `0x21`, or `0x0A` then `0x21`) into the Block Header. At most one pre-filter is allowed (any two together, or any three-filter chain, is rejected).
...
- `bcj_arm` decodes `xz --arm` output byte-for-byte; `bcj_arm64` matches `lzma_bcj_arm64_encode/decode` byte-for-byte on embedded vectors and decodes an `xz --arm64` file; both encoders' output is accepted by `xz -d` / Python `lzma`
- Unknown `.xz` filter ID does not silently skip
- XZ Delta/x86/ARM/ARM64 may appear only before the final LZMA2 filter; any of them as the last filter is `Data`; x86/ARM/ARM64 properties must be empty; Delta properties must be one byte
...
Per-ISA BCJ container filters (ARM-Thumb/PowerPC/IA64/SPARC) and x86/ARM/ARM64 combined with Delta in one chain are later work.
